...
def get_sensor_data(ser):
    global sensor_data, state, sensor_stop, is_serial_stopped
...
def update_position() :
    global position, update_stop, tiltheading, td
    while update_stop : 
        if td : 
            position[0] = td * math.cos(math.radians(tiltheading))
            position[1] = td * math.sin(math.radians(tiltheading))
            time.sleep(1)
...
def main():
    global lidar_data, sensor_data, rplidar, ser, state, sensor_stop, tiltheading, td, position
    ...
    try:
        while True:
            try : 
                ...
                dist1, dist2, lidar, heading, tiltheading, td = sensor_data
                tiltheading -= 115
                td *= -0.001
                ...
                if is_print : 
                    print(
                            ...
                            f"tiltheading: {round(tiltheading, 4) if tiltheading is not None else 0:<10}", end=""
                        )
