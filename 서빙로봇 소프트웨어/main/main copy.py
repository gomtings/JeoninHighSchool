...
def print_map(grid_map, undefined_points) : 
...
def main() : 
    file_path = "AutoServingRobot/Software/1/lidar.csv"
    ang, dist = get_data(file_path)
    mapped_data, center_x, center_y = lidar_to_grid_map.mapping(0.2, ang, dist)
    mapped_data[center_x][center_y] = 3
    simplified_map = map_simplify(mapped_data)
    ...
    print_map(simplified_map, undefined_points)
