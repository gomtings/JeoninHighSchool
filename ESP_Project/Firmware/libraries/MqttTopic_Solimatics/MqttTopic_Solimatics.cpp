...
void MqttTopic::PublishTopicYJSensing(void)
{
  ...
  JsonArray holl = doc.createNestedArray("Holl");
  for(int i=0;i<arraynum;i++)
  {
    holl.add(qyjsensing.Holl[i]);
  }
  serializeJson(doc, qbuf);
}
...
void MqttTopic::ParsingTopicAnalysisData(byte* payload, unsigned int length)
{
  char* p = (char*)malloc(length);
  memcpy(p,payload,length);
  ...
  StaticJsonDocument<256> doc;
  DeserializationError error = deserializeJson(doc, p);
  analysisdata.BitPosition = doc["BEAT_POSITION"];
  analysisdata.MaxDepth = doc["MAX_DEPTH"];
  analysisdata.Encod_low = doc["Encod_low"];
  analysisdata.RodCount = doc["AttachRodCount"];
  analysisdata.Holl = doc["Holl"];
  free(p);
}
...
void MqttTopic::ParsingTopicRequestLogin(byte* payload, unsigned int length)
{
  ...
  qrequestlogin.model = doc["Model_Code"];
...
void MqttTopic::ParsingTopicRequestStart(byte* payload, unsigned int length)
{
  ...
  qrequeststart.model = doc["Model_Code"];
