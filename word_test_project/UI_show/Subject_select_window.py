...
class Subject_select_window(QMainWindow,Ui_Subject):
    def __init__(self,parents,Exam_record_path,Wrong_list_path,Workbook_path,Base_path,name):
    ...
    def open_test_Window(self):
        if self.test_Window is None or not self.test_Window.isVisible():
            self.hide()
            self.test_Window = test_Window(self,self.select_day,self.Exam_record_path,self.Wrong_list_path,self.Workbook_path,self.Base_path,self.name) 
