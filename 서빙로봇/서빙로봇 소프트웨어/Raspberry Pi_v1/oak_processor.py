...
class OakProcessor:
    ...
    def _try_reconnect(self):
        """연결 끊김 시 3초 쿨다운 후 자동 재연결 시도"""
        import time
        now = time.time()
        if now - getattr(self, "_last_reconnect_time", 0.0) < 3.0:
            return
        self._last_reconnect_time = now

        print("[OAK] 🔄 카메라 재연결 시도 중...")
        self.stop()
        try:
            self.start()
            print("[OAK] ✅ 카메라 자동 재연결 성공!")
        except Exception as e:
            print(f"[OAK] 재연결 대기 중 ({e})")
    ...
    def get_frame(self) -> Optional[OakFrame]:
        ...
        try:
            ...
            # USB 순간 끊김, X_LINK_ERROR 등 통신 예외 발생 시 자원 정리 후 재연결 트리거
            self.stop()
            self._try_reconnect()
            return None
