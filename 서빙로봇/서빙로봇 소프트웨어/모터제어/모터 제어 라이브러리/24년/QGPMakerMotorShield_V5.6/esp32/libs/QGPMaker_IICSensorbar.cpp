...
SensorBar::SensorBar(byte address, byte resetPin, byte interruptPin, byte oscillatorPin)
{
...
byte SensorBar::begin(byte address, byte resetPin)
{
	// Store the received parameters into member variables
	deviceAddress = address;
	pinReset = resetPin;
...
void SensorBar::pinDir(byte pin, byte inOut)
{
...
void SensorBar::pinMode(byte pin, byte inOut)
{
	pinDir(pin, inOut);
}

void SensorBar::writePin(byte pin, byte highLow)
{
	unsigned int tempRegDir = readWord(REG_DIR_B);
...
void SensorBar::digitalWrite(byte pin, byte highLow)
{
	writePin(pin, highLow);
}

byte SensorBar::readPin(byte pin)
{
	unsigned int tempRegDir = readWord(REG_DIR_B);
...
byte SensorBar::digitalRead(byte pin)
{
	return readPin(pin);
}

void SensorBar::ledDriverInit(byte pin, byte freq /*= 1*/, bool log /*= false*/)
{
...
void SensorBar::pwm(byte pin, byte iOn)
{
...
void SensorBar::analogWrite(byte pin, byte iOn)
{
	pwm(pin, iOn);
}

void SensorBar::blink(byte pin, unsigned long tOn, unsigned long tOff, byte onIntensity, byte offIntensity)
{
...
void SensorBar::breathe(byte pin, unsigned long tOn, unsigned long tOff, unsigned long rise, unsigned long fall, byte onInt, byte offInt, bool log)
{
...
void SensorBar::setupBlink(byte pin, byte tOn, byte tOff, byte onIntensity, byte offIntensity, byte tRise, byte tFall, bool log)
{
...
void SensorBar::keypad(byte rows, byte columns, unsigned int sleepTime, byte scanTime, byte debounceTime)
{
	unsigned int tempWord;
	byte tempByte;
...
unsigned int SensorBar::readKeypad()
{
	return readKeyData();
}

unsigned int SensorBar::readKeyData()
{
	return (0xFFFF ^ readWord(REG_KEY_DATA_1));
}

byte SensorBar::getRow(unsigned int keyData)
{
...
byte SensorBar::getCol(unsigned int keyData)
{
...
void SensorBar::debounceConfig(byte configValue)
{
...
void SensorBar::debounceTime(byte time)
{
...
void SensorBar::debounceEnable(byte pin)
{
...
void SensorBar::debouncePin(byte pin)
{
	debounceEnable(pin);
}

void SensorBar::debounceKeypad(byte time, byte numRows, byte numCols)
{
...
void SensorBar::enableInterrupt(byte pin, byte riseFall)
{
...
unsigned int SensorBar::interruptSource(bool clear /* =true*/)
{
...
bool SensorBar::checkInterrupt(int pin)
{
...
void SensorBar::clock(byte oscSource, byte oscDivider, byte oscPinFunction, byte oscFreqOut)
{
	configClock(oscSource, oscPinFunction, oscFreqOut, oscDivider);
}

void SensorBar::configClock(byte oscSource /*= 2*/, byte oscPinFunction /*= 0*/, byte oscFreqOut /*= 0*/, byte oscDivider /*= 1*/)
{
...
byte SensorBar::calculateLEDTRegister(int ms)
{
	int regOn1, regOn2;
...
byte SensorBar::calculateSlopeRegister(int ms, byte onIntensity, byte offIntensity)
{
...
byte SensorBar::readByte(byte registerAddress)
{
	byte readValue;
...
unsigned int SensorBar::readWord(byte registerAddress)
{
...
//	- No return value.
void SensorBar::readBytes(byte firstRegisterAddress, byte *destination, byte length)
{
	byte readValue;
...
//	- No return value.
void SensorBar::writeByte(byte registerAddress, byte writeValue)
{
	Wire.beginTransmission(deviceAddress);
	Wire.write(registerAddress);
...
//	- No return value.
void SensorBar::writeWord(byte registerAddress, unsigned int writeValue)
{
	byte msb, lsb;
...
//	- no return value.
void SensorBar::writeBytes(byte firstRegisterAddress, byte *writeArray, byte length)
{
	Wire.beginTransmission(deviceAddress);
	Wire.write(firstRegisterAddress);
	for (int i = 0; i < length; i++)
